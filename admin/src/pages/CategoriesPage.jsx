    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card className="p-4 flex flex-col items-center justify-center">
          ...
          <div className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider mt-2">Total Categories</div>
      ...
      {/* Categories Table */}
      <Card>
        <CardHeader title="Category List" subtitle={`${filteredCategories.length} categories found`} />
          ...
          <div className="text-center py-4 space-y-4">
            <div className="flex justify-center">
              <div className="w-14 h-14 rounded-full bg-indigo-100 flex items-center justify-center">
                <span className="text-lg font-bold text-indigo-700">{selectedCategory.name?.charAt(0) || 'C'}</span>
              </div>
            </div>
            <p className="text-base font-bold text-slate-900">{selectedCategory.name}</p>
            <p className="text-xs font-bold text-slate-500 -mt-2">Category Information</p>
            <div className="space-y-2">
              <div className="flex items-center justify-between px-2 py-2">
                <span className="text-xs font-bold text-slate-900 uppercase tracking-wider">Slug</span>
                <span className="text-sm text-slate-700 font-mono">{selectedCategory.slug || '-'}</span>
              </div>
              <div className="flex items-center justify-between px-2 py-2">
                <span className="text-xs font-bold text-slate-900 uppercase tracking-wider">Status</span>
                <Badge tone={statusTone(selectedCategory.status)} dot>{selectedCategory.status}</Badge>
              </div>
            </div>
          </div>
        )}
      ...
      <Modal 
        ...
        subtitle={selectedCategory ? 'Update category information' : 'Add a new category'}
        footer={
          <>
      ...
      >
        <form id="category-form" onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            />
            <p className="text-xs text-slate-400 mt-1">URL-friendly version of the category name</p>
          </div>
          {selectedCategory && (
            <div className="space-y-1">
        ...
        {deleteTarget && (
          <div className="text-center py-4">
            ...
            <p className="text-xs text-slate-500">
              This will permanently remove this category<br />and all associated data from the system.
            </p>
          </div>
        )}
      </Modal>
